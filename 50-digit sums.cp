...
int main()
{
    ...
    for (int y = 49; y >= 0; y--)
    {
        //loads sum of every column into carry array, everything seems to be good in debugger, tested three columns to be accurate by hand.
        temp = 0;
        for (int x = 0; x < 100; x ++)
        {
        temp += data[x][y];
    ...
    {
        //load answer array
        //takes the 1's digit of the sum of the current column and puts it into the answer
        answer[k] = carry[z] % 10;
        //eliminates the 1's place due to integer division
        carry[z] /= 10;
        //adds the two digit carry to the next column
        carry[z-1] += carry[z];
        k--;
    }
    
    //loads the first three digits into the first element of the answer array
    answer[k] = carry[0];
    for (k = 0; k <= 49; k++)
    {
        cout << answer[k];
